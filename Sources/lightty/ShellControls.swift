...
final class ShellFieldBox: NSView {
    ...
    private var editing = false { didSet { applyLook() } }
    ...
    init(_ field: NSTextField) {
        ...
        // 没有系统焦点环了，聚焦得自己说一声。NSControl 发的这两条带的是控件本身。
        for name in [NSControl.textDidBeginEditingNotification, NSControl.textDidEndEditingNotification] {
            NotificationCenter.default.addObserver(
                self, selector: #selector(editingChanged(_:)), name: name, object: field)
        }
        applyLook()
    }
    ...
    @objc private func editingChanged(_ note: Notification) {
        editing = note.name == NSControl.textDidBeginEditingNotification
    }
