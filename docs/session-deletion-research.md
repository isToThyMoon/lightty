...
Only internal terminals and pending resumes for the exact target composite key block deletion; an idle shell does not. Lightty resume/picker launches are excluded while confirmation/deletion is active. Claude process inspection uses the existing hook association (PID plus kernel start time and composite session key): known unrelated sessions are allowed, the target is rejected with its PID, and an unreadable live-session table produces a distinct unknown-occupancy message. Command-line resume IDs are not treated as current-session evidence because an Agent can switch conversations after launch. This check is not an atomic lock and cannot identify every custom wrapper; the confirmation asks users not to start the Agent elsewhere during deletion. Native Codex writer-lock rejection, including occupied descendants, remains authoritative.

Claude process identity (updated 2026-09-16): process-table inspection has been removed. `ClaudeSessionProvider.checkDeletable` now trusts only two positive sources — Claude's own live-session table (`claude agents --json`) and Lightty's verified pane associations (PID plus kernel start time). The target in the live table is occupied with its PID; a pane of this app running the target is occupied; an unreadable live table is unknown with no PID; anything else passes. Identifying `claude` processes required matching an executable name that changes with the install method (a version number natively, `claude.exe` under npm), so a miss silently turned "in use" into "idle"; with it gone, an external Claude that has not registered yet is simply invisible and the registration wait is no longer needed. The live table is injected through `ClaudeSessionProvider.DeletionProbe`, so `SessionDeletionTests` covers these cases without starting Claude.
