...
Claude process identity (updated 2026-09-12): `ClaudeSessionProvider.checkDeletable` first maps PIDs through Claude's own live-session table (`claude agents --json`), then overlays Lightty's verified pane associations, and every `claude` process listed by `ps` must be identified this way. A freshly started Claude is not in that table yet (measured locally with Claude 2.1.270: about 2.2 s from kernel start time to registration), so it used to be reported as unknown. Now, when every unidentified process started less than `registrationWindow` (5 s) ago, the check records each process's kernel identity, waits `registrationWait` (2 s) once, and rereads the table once — no polling. A process that registered another session is allowed; one that registered the target is occupied; one still missing, or whose PID now belongs to a different process, stays unknown. If any unidentified process is older than the window, if its start time cannot be read, or if the first live-table read fails, the result is unknown without waiting; a failed reread is also unknown. Consent to "Permanently delete anyway" still suppresses only unknown occupancy, never an occupied result found after the wait. The process table, live table, process identity, clock and wait are injected through `ClaudeSessionProvider.DeletionProbe`, so `SessionDeletionTests` covers these cases without starting Claude or sleeping. The wait runs only on the background queue `SessionDeletion.perform` dispatches to.
