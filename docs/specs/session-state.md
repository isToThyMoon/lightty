...
- pane 显示的标题同一时刻只有一个来源，来源之间不穿插，按这家的 hook 插件装没装
  （`HookInstaller.installedVersion`，lightty 自己的台账）分两条路：
  - 装了：标题只看 hook + 官方目录。agent 启动到第一发 hook 之间是 pane 名，绑定后是目录标题
    （目录未读到时仍是 pane 名），`SessionEnd` 直接回 pane 名。agent 写的终端标题（OSC 0）在这条路上
    只贡献三样：图标（启动阶段就认出是哪家，Codex 到第一句提交才跑 `SessionStart`）、忙/闲这条边、
    以及标题正文变了就催目录重读。
  - 没装：终端标题的形状看得出是哪家 agent 写的（`AgentSpec.terminalTitle` 里带前缀的形状）就照原文
    显示，和原生终端一样，图标按认出的家给，pane 头的改名只读（认不准那是不是能收 `/rename` 的会话）；
    agent 退出（shell 集成的命令结束标记）后回到 pane 名。
  shell 写的标题两条路都不显示，标签页名不受 OSC 影响。Codex 装好后还要用户批准一次信任，
  批没批准 lightty 看不到，没批准的表现是「图标 + pane 名」一直不变。
...
7. Agent 操作可使元数据失效。模型合并延迟读取，最多五次；取消同时清除待重试请求。
   ...
   使元数据失效的时机只有三处：关联变化、钩子报回合结束（`.done`）、状态从非 thinking 变 thinking
   （用户开始下一轮）。最后一处是为了用户自己在终端里敲的 `/rename`——那不触发任何钩子，
   他改完名通常紧接着提问，标题因此不必等这一轮结束。不监听会话记录等私有文件。
