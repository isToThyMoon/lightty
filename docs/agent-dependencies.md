...
| Agent 进程身份、退出检测 | hook 记下祖先链上最近的**前台作业组长**（`kinfo_proc` 的 `e_pgid` / `e_tpgid` / `e_tdev`）的 pid + 启动时间；lightty 用 `DispatchSource` 监视退出 | 同左；走会话路由记录时用记录里的界面进程 | npm 版 codex 的 node 包装、用户的启动脚本，组长就是包装进程，监视它等价；没有 pty 时身份留空，退化为只靠 `SessionEnd` |
| 共享后台进程里的 hook 属于哪个 pane | 不涉及 | 0.157 起会话跑在共用的 app-server 后台进程里，hook 继承的 `LIGHTTY_PANE_ID` 属于拉起它的终端。lightty 经 `codex app-server proxy`（官方入口）旁听 `thread/started` / `thread/status/changed` / `thread/closed`，按进程事实把会话对到 pane，写 `~/.lightty/run/sessions/<session_id>`；hook 先查这条记录 | 广播字段变了或 `proxy` 不可用：写不出记录，hook 查不到就静默，Codex 会话没有状态但不会送错 pane。规则与边界见 [hooks 文档](hooks.md#codex-共享后台进程) |
