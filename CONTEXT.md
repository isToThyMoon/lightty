...
**窗口排布（Window Arrangement）**：一个窗口里有哪些标签页、按什么顺序、当前是哪一个，以及每个标签页的分屏排布、标签页名、放大中的终端和最近聚焦的终端。标签页名要么是默认名（只记序号，显示时按当前语言格式化，序号跨窗口不重复），要么是用户起的名字。标签页与终端一律按身份指代，序号只是显示位次；关闭、移动、拆出、撤销都是从一份完整的排布换到另一份，不存在改到一半的中间态。
_Avoid_：用序号指代标签页（列表随时在变）、把视图层级或 NSSplitView 当成排布本身、把放大态或焦点记在视图上。
